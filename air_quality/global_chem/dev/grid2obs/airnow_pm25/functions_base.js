...
function showImage(){
        ...
        // document.map_image.onerror = function(){remove_loading(this.varid,this.id);this.src='/home/people/emc/www/htdocs/users/verification/global/gcafs/test/grid2obs/images/noimage.png';};
        document.map_image.onerror = function(){remove_loading(this.varid,this.id);this.src='../images/noimage.png';};
...
*/        
        ...
        for (var i1=0; i1<frames.length; i1++){
                ...
                plottypes[idx_var].images[i].onerror = function(){remove_loading(this.varid,this.id);this.src='../images/noimage.png';};
...
function changePlotType(id){
        ...
        // Adjust Valid Hours List by PlotType
        //
        var selected_validhour = document.getElementById("validhour").value;
        var element = document.getElementById("validhour");
        for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
        //
	if(selected_plottype=="timeseries"&&selected_metric=="fbar_obar"){
            plottype_validhours = fbar_obar_validhours;
            plottype_validhours_name = fbar_obar_validhours_name;
        }
	else if(selected_plottype=="vhrmean"){
            ...
            plottype_validhours = common_validhours;
            plottype_validhours_name = common_validhours_name;
        }
        validhours = [];
        ...
        //
        // Adjust Forecast Hours List by plottype
        //
	...
	if(selected_plottype=="timeseries"&&selected_metric=="fbar_obar"){
            ...
            plottype_forecasthours_name = fbar_obar_forecasthours_name;
        }
	else if(selected_plottype=="fhrmean"){
            ...
            plottype_forecasthours_name = vhrmean_forecasthours_name;
        }
        else{
            if(selected_validhour=="_valid00z"){
                plottype_forecasthours = vhr00_forecasthours;
                plottype_forecasthours_name = vhr00_forecasthours_name;
            }
            else if(selected_validhour=="_valid06z"){
                plottype_forecasthours = vhr06_forecasthours;
                plottype_forecasthours_name = vhr06_forecasthours_name;
            }
            else if(selected_validhour=="_valid12z"){
                plottype_forecasthours = vhr12_forecasthours;
                plottype_forecasthours_name = vhr12_forecasthours_name;
            }
            else if(selected_validhour=="_valid18z"){
                plottype_forecasthours = vhr18_forecasthours;
                plottype_forecasthours_name = vhr18_forecasthours_name;
            }
            else{
                plottype_forecasthours = common_forecasthours;
                plottype_forecasthours_name = common_forecasthours_name;
            }
        }
        forecasthours = [];
	...
	else if(selected_metric=="csi"&&(selected_plottype=="timeseries"||selected_plottype=="fhrmean")){
...
function changeMetric(id){
        ...
        var selected_metric = document.getElementById("metric").value;
        //
        // Adjust Valid Hours List by metric
        //
        var selected_validhour = document.getElementById("validhour").value;
        var element = document.getElementById("validhour");
        for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
        //
	if(selected_plottype=="timeseries"&&selected_metric=="fbar_obar"){
            metric_validhours = fbar_obar_validhours;
            metric_validhours_name = fbar_obar_validhours_name;
        }
	else if(selected_plottype=="vhrmean"){
            metric_validhours = vhrmean_validhours;
            metric_validhours_name = vhrmean_validhours_name;
        }
        else{
            metric_validhours = common_validhours;
            metric_validhours_name = common_validhours_name;
        }
        validhours = [];
	...
	if(selected_metric=="ctc"){
        ...
        element.onchange();
        //
        // Adjust Forecast Hours List by metric
        //
        var selected_validhour = document.getElementById("validhour").value;
        var selected_forecasthour = document.getElementById("forecasthour").value;
        var element = document.getElementById("forecasthour");
        ...
        if(selected_plottype=="fhrmean"){
            metric_forecasthours = fhrmean_forecasthours;
            metric_forecasthours_name = fhrmean_forecasthours_name;
        }
	else if(selected_plottype=="vhrmean"){
            metric_forecasthours = vhrmean_forecasthours;
            metric_forecasthours_name= vhrmean_forecasthours_name;
        }
	else if(selected_metric=="fbar_obar"){
            metric_forecasthours = fbar_obar_forecasthours;
            metric_forecasthours_name= fbar_obar_forecasthours_name;
        }
        else{
            if(selected_validhour=="_valid00z"){
                metric_forecasthours = vhr00_forecasthours;
                metric_forecasthours_name = vhr00_forecasthours_name;
            }
            else if(selected_validhour=="_valid06z"){
                metric_forecasthours = vhr06_forecasthours;
                metric_forecasthours_name = vhr06_forecasthours_name;
            }
            else if(selected_validhour=="_valid12z"){
                metric_forecasthours = vhr12_forecasthours;
                metric_forecasthours_name = vhr12_forecasthours_name;
            }
            else if(selected_validhour=="_valid18z"){
                metric_forecasthours = vhr18_forecasthours;
                metric_forecasthours_name = vhr18_forecasthours_name;
            }
            else{
                metric_forecasthours = common_forecasthours;
                metric_forecasthours_name = common_forecasthours_name;
            }
        }
        forecasthours = [];
        for(i=0; i<metric_forecasthours.length; i++){
            forecasthours.push({
                displayName: metric_forecasthours_name[i],
...
function changeValidHour(id){
	...
	if(selected_plottype=="timeseries"&&selected_metric=="fbar_obar"){
            validhour_forecasthours = fbar_obar_forecasthours;
            validhour_forecasthours_name= fbar_obar_forecasthours_name;
        }
	else if(selected_plottype=="fhrmean"){
            validhour_forecasthours = fhrmean_forecasthours;
            validhour_forecasthours_name = fhrmean_forecasthours_name;
        }
	else if(selected_plottype=="vhrmean"){
            validhour_forecasthours = vhrmean_forecasthours;
            validhour_forecasthours_name = vhrmean_forecasthours_name;
        }
        else{
            if(selected_validhour=="_valid00z"){
                validhour_forecasthours = vhr00_forecasthours;
                validhour_forecasthours_name = vhr00_forecasthours_name;
            }
            else if(selected_validhour=="_valid06z"){
                validhour_forecasthours = vhr06_forecasthours;
                validhour_forecasthours_name = vhr06_forecasthours_name;
            }
            else if(selected_validhour=="_valid12z"){
                validhour_forecasthours = vhr12_forecasthours;
                validhour_forecasthours_name = vhr12_forecasthours_name;
            }
            else if(selected_validhour=="_valid18z"){
                validhour_forecasthours = vhr18_forecasthours;
                validhour_forecasthours_name = vhr18_forecasthours_name;
            }
            else{
                validhour_forecasthours = common_forecasthours;
                validhour_forecasthours_name = common_forecasthours_name;
            }
        }
        forecasthours = [];
        for(i=0; i<validhour_forecasthours.length; i++){
            forecasthours.push({
                displayName: validhour_forecasthours_name[i],
                name: validhour_forecasthours[i],
            })
        }
        for(i=0; i<forecasthours.length; i++){
            var option = document.createElement("option");
