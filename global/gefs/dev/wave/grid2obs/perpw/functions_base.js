...
function getURL(plottype,metric,obstype,daterange,validhour,forecasthour,region,frame){
...
function showImage(){
	...
	var url = getURL(imageObj.plottype,imageObj.metric,imageObj.obstype,imageObj.daterange,imageObj.validhour,imageObj.forecasthour,imageObj.region,i);
...
function changeObstype(id){
	...
	var selected_obstype = document.getElementById("obstype").value;

        //Region
        var selected_region = document.getElementById("region").value;
        var element = document.getElementById("region");
        for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
        if(selected_obstype=="sfcshp"){
                obstype_regions = sfcshp_regions;
                obstype_regions_name = sfcshp_regions_name;
        }
        else if(selected_obstype=="ndbc_standard"){
                obstype_regions = ndbc_standard_regions;
                obstype_regions_name = ndbc_standard_regions_name;
        }
        regions = [];
        for(i=0; i<obstype_regions.length; i++){
        regions.push({
                displayName: obstype_regions_name[i],
                name: obstype_regions[i],
                })
        }
        for(i=0; i<regions.length; i++){
        var option = document.createElement("option");
