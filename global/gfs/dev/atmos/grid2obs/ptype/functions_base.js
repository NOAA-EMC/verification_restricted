...
function getURL(plottype,metric,daterange,validhour,forecasthour,region,variable,frame){
...
function showImage(){
	...
	var url = getURL(imageObj.plottype,imageObj.metric,imageObj.daterange,imageObj.validhour,imageObj.forecasthour,imageObj.region,imageObj.variable,i);
...
function changeValidHour(id){
        ...
        var selected_forecasthour = document.getElementById("forecasthour").value;
        var element = document.getElementById("forecasthour");
        og_forecasthours = timeseries_forecasthours;
        if(selected_validhour=="03"||selected_validhour=="09"||selected_validhour=="15"||selected_validhour=="21"){
                validhour_forecasthours = [];
                for(i=0; i<og_forecasthours.length; i++){
                        if(parseInt(og_forecasthours[i])+3 <= 60){
                            validhour_forecasthours[i]=NumToString(parseInt(og_forecasthours[i])+3);
                        }
                }
        }
        else {
                validhour_forecasthours = og_forecasthours;
        }
        if(selected_plottype=="timeseries"){
                for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
                forecasthours = [];
                for(i=0; i<validhour_forecasthours.length; i++){
                        forecasthours.push({
                            displayName:  "F"+validhour_forecasthours[i],
