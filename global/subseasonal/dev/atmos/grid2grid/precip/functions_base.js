...
function populateMenu(mode){
	...
	else if(mode == 'avgtype'){
		var element = document.getElementById("avgtype");
		for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}

		for(i=0; i<avgtypes.length; i++){
			var option = document.createElement("option");
			option.text = avgtypes[i].displayName;
			option.value = avgtypes[i].name;
			element.add(option);
		}
	}
	else if(mode == 'metric'){
...
function getURL(plottype,avgtype,metric,daterange,validhour,forecasthour,region,threshold,frame){
	var newurl = url.replace("PPP",plottype);
	var newurl = newurl.replace("AAA",avgtype);
...
function showImage(){
	...
	var url = getURL(imageObj.plottype,imageObj.avgtype,imageObj.metric,imageObj.daterange,imageObj.validhour,imageObj.forecasthour,imageObj.region,imageObj.threshold,i);
	...
	document.getElementById("avgtype").selectedIndex = searchByName(imageObj.avgtype,avgtypes);
...
function changePlotType(id){
	...
	else if(selected_plottype=="perfdia"){
	...
	//Avg Type
	var selected_avgtype = document.getElementById("avgtype").value;
	var element = document.getElementById("avgtype");
	for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
	if(selected_plottype=="fhrmean"){
		plottype_avg = fhrmean_avg;
		plottype_avg_name = fhrmean_avg_name;
	}
	else{
		plottype_avg = non_fhrmean_avg;
		plottype_avg_name = non_fhrmean_avg_name;
	}
	avgtypes = [];
	for(i=0; i<plottype_avg.length; i++){
	avgtypes.push({
		displayName:  plottype_avg_name[i],
		name: plottype_avg[i],
	        })
	}
	for(i=0; i<avgtypes.length; i++){
	...
	var avgtypes_values= [];
	for(i=0; i<element.options.length; i++){
	       avgtypes_values.push(element.options[i].value);
	}
	if(avgtypes_values.indexOf(selected_avgtype) != -1){
	       var idx = avgtypes_values.indexOf(selected_avgtype);
	       element.options[idx].selected = true;
	       element.onchange();
	}
	else{
	...
	var element = document.getElementById("threshold");
	for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
	if( (selected_metric=="me") || (selected_metric=="rmse")){
...
//Change the average type from dropdown menu
function changeAverageType(id){
	imageObj.avgtype = id;
	preload(imageObj);
	showImage();
	document.getElementById("avgtype").blur();
	...
	var element = document.getElementById("forecasthour");
	for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
	if(selected_avgtype=="days6_10"){
		if(selected_plottype=="timeseries"){
			avgtype_forecasthours = days610_forecasthours;
		}
		else if(selected_plottype=="perfdia"){
		        avgtype_forecasthours = days610_forecasthours;
		}
	}
	else if(selected_avgtype=="weekly"){
		if(selected_plottype=="timeseries"){
			avgtype_forecasthours = weekly_forecasthours;
		}
		else if(selected_plottype=="fhrmean"){
			avgtype_forecasthours = fhrmean_forecasthours;
		}
		else if(selected_plottype=="perfdia"){
			avgtype_forecasthours = weekly_forecasthours;
		}
	}
	else if(selected_avgtype=="weeks3_4"){
		if(selected_plottype=="timeseries"){
		        avgtype_forecasthours = weeks34_forecasthours;
		}
		else if(selected_plottype=="perfdia"){
			avgtype_forecasthours = weeks34_forecasthours;
		}
	...
	forecasthours = [];
	for(i=0; i<avgtype_forecasthours.length; i++){
	if(selected_plottype=="fhrmean"){
	forecasthours.push({
		displayName: "To F"+avgtype_forecasthours[i],
		name: avgtype_forecasthours[i],
	        })
	}
	else{
	forecasthours.push({
		displayName:  "F"+avgtype_forecasthours[i],
		name: avgtype_forecasthours[i],
	        })
	}
	}
	...
	for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
	if( (selected_metric=="me") || (selected_metric=="rmse")){
		avgtype_thresholds = me_thresholds;
		avgtype_thresholds_name = me_thresholds_name;
	}
	else{
	        if(selected_avgtype=="days6_10"){
		        if(selected_plottype=="timeseries"){
			        avgtype_thresholds = days610_thresholds;
			        avgtype_thresholds_name = days610_thresholds_name;
		        }
		        else if(selected_plottype=="perfdia"){
			        avgtype_thresholds = perfdiag_thresholds;
			        avgtype_thresholds_name = perfdiag_thresholds_name;
		        }
	        }
	        else if(selected_avgtype=="weekly"){
		        if(selected_plottype=="timeseries"){
			        avgtype_thresholds = weekly_thresholds
			        avgtype_thresholds_name = weekly_thresholds_name;
		        }
		        else if(selected_plottype=="fhrmean"){
			        avgtype_thresholds = weekly_thresholds;
			        avgtype_thresholds_name = weekly_thresholds_name;
		        ...
		        if(selected_plottype=="timeseries"){
			        avgtype_thresholds = weeks34_thresholds;
			        avgtype_thresholds_name = weeks34_thresholds_name;
		        }
		        else if(selected_plottype=="perfdia"){
			        avgtype_thresholds = perfdiag_thresholds;
			        avgtype_thresholds_name = perfdiag_thresholds_name;
	...
	thresholds = [];
	for(i=0; i<avgtype_thresholds.length; i++){
	thresholds.push({
		displayName: avgtype_thresholds_name[i],
		name: avgtype_thresholds[i],
	        })
	}
	for(i=0; i<thresholds.length; i++){
	var option = document.createElement("option");
...
function changeMetric(id){
	...
	if( (selected_metric=="me") || (selected_metric=="rmse")){
		if(selected_plottype=="timeseries"){
			if(selected_avgtype=="days6_10"){
		                metric_thresholds = me_thresholds;
		                metric_thresholds_name = me_thresholds_name;
			}
			else if(selected_avgtype=="weekly"){
				metric_thresholds = me_thresholds;
				metric_thresholds_name = me_thresholds_name;
		...
		if(selected_plottype=="perfdia"){
			...
			if(selected_avgtype=="days6_10"){
				metric_thresholds = days610_thresholds;
				metric_thresholds_name = days610_thresholds_name;
			}
			else if(selected_avgtype=="weekly"){
				metric_thresholds = weekly_thresholds
				metric_thresholds_name = weekly_thresholds_name;
			}
			else if(selected_avgtype=="weeks3_4"){
				metric_thresholds = weeks34_thresholds;
				metric_thresholds_name = weeks34_thresholds_name;
			}
		}
		else if(selected_plottype=="fhrmean"){
			if(selected_avgtype=="weekly"){
